...
@description('Conditional. The resource ID of a key vault to reference a customer managed key for encryption .')
...
resource cognitiveService 'Microsoft.CognitiveServices/accounts@2023-05-01' = {
  ...
  properties: {
    ...
    publicNetworkAccess: !empty(publicNetworkAccess) ? publicNetworkAccess : (!empty(networkAcls) ? 'Enabled' : 'Disabled')
...
resource cognitiveService_roleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for (roleAssignment, index) in (roleAssignments ?? []): {
